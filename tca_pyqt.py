...
class MainWin(QMainWindow):
    ...
    def initUI(self):
        ...
        SegmentAction.triggered.connect(self.seg_gui)
        ...
        CoregisterAction.triggered.connect(self.coreg_gui)
        ...
        if self.lan == 0:
            ExtractAction = QAction('&ROI Extract', self)
            ExtractAction.setStatusTip('Extract data from selected ROIs')
        elif self.lan == 1:
            ExtractAction = QAction('区域提取', self)
            ExtractAction.setStatusTip('将所选区域的数据进行提取')
        ExtractAction.setShortcut('Ctrl+R')
        ExtractAction.triggered.connect(self.Extract_gui)
        ...
        calMenu.addAction(ExtractAction)
    ...
    # 打开文件，只能打开nii或nii.gz后缀的文件，否则无法读取
        ...
        if self.lan == 0:
            ...
            if platform.system() == "Windows":
                self.fname = QFileDialog.getOpenFileNames(self, '打开文件', './', "Nii文件 (*.nii;*.nii.gz);;所有文件 (*)")[0]
            elif platform.system() == "Linux":
                self.fname = QFileDialog.getOpenFileNames(self, '打开文件', './')[0]
            else:
                self.fname = QFileDialog.getOpenFileNames(self, '打开文件', './')[0]
        if self.fname == []:
            self.fname = ""
    ...
    def coreg_gui(self):
        self.ui_coreg = window_coreg(self, self.lan)
        self.ui_coreg.show()

    def seg_gui(self):
        self.ui_seg = window_seg(self, self.lan)
        self.ui_seg.show()

    def Extract_gui(self):
        self.ui_extract = ui_extract(self, self.lan)
        self.ui_extract.show()
